...
test('OpenAPI JSON documents auth and onboarding endpoints', async () => {
  ...
  assert.ok(response.body.paths['/api/v1/auth/otp/send']);
  assert.ok(response.body.paths['/api/v1/auth/otp/verify']);
  assert.ok(response.body.paths['/api/v1/auth/staff/login']);
  assert.ok(response.body.paths['/api/v1/auth/refresh']);
  assert.ok(response.body.paths['/api/v1/auth/onboarding']);
  assert.ok(response.body.paths['/api/v1/users/me']);
