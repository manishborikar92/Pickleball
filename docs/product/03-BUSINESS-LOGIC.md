...
Routes are guarded by **specific permissions**, not role names. The backend checks permission keys (like `edit_pricing`) contextually per venue. This keeps the access control layer stable.

For the authoritative technical roles & permissions mapping matrix, session configuration variables, and security constraints, refer to the technical specification [docs/architecture/06-BUSINESS-RULES.md](../architecture/06-BUSINESS-RULES.md).

### 1.2 Contextual Venue Scoping

Roles are assigned per venue via `venue_user_roles`. All operator queries are filtered to the venue(s) they have role assignments for. The scoping model supports multiple venues natively without structural changes.
