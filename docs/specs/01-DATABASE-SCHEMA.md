...
| `type` | VARCHAR(50) | NOT NULL | Enum: `scratch_card`, `spinner`, `coupon_drop`, `points`. Extensible — new types added as new enum values. Mechanism creation currently accepts `scratch_card` and `spinner` (the types with launch support). |
...
**`config` JSONB — `spinner` type (same prize schema, plus wheel layout):**
```json
{
  "segment_count": 8,
  "prizes": [
    { "id": "s1", "label": "Try Again",        "type": "no_prize", "probability": 0.50 },
    { "id": "s2", "label": "Free Iced Coffee", "type": "voucher",  "validity_days": 14, "probability": 0.50 }
  ]
}
```
