...
- [x] **Commit-on-Confirm Checkout & Hybrid Persistence (web)**: Nothing is reserved during review; `checkoutBookingAction` runs hold → waiver → initiate-payment in one server pass at the final Confirm & Pay click (`runCheckout` in `web/src/lib/services/checkout.js`). `/venues/[slug]/book` uses the **Hybrid Persistence Architecture**: selected dates sync to URL search params (`?date=YYYY-MM-DD`) for deep-linking and SSR pre-fetching in `BookPage`; draft court selections, time ranges, and coupons persist in `sessionStorage` (`pb:draft:${venueId}`, version: 1, 2-hour TTL) with live availability intersection on mount; active holds persist under `pb:hold:${venueId}`. `useCountdown` drives the live 10-minute payment countdown, allowing reloads and gateway returns to resume payment (`confirmHeldBookingAction`). Expiry releases local state and returns the user to the grid.
