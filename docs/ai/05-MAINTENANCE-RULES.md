...
To prevent conflicting updates, we enforce strict ownership boundaries for all documentation files:
...
- **Technical & Integration Specifications (`docs/specs/`, `docs/integrations/`)**: Maintained by the Technical Lead or Solutions Architect.
...
- **Handling Divergence**: When implementation differs from specifications, the AI context must document the divergence in `docs/ai/03-IMPLEMENTATION-STATUS.md` rather than assuming either source is correct. Do not update product specifications or assume the codebase is wrong without explicit verification from the PO.
...
- **New Architecture Decisions**: Add a file to `docs/adrs/` and add a new row in the ADR registry section of `docs/README.md`.
...
- **Link Audits**: Developers must check that relative markdown links between relocated files resolve correctly before merging PRs. Standard markdown linting tools or IDE extensions can be used to run link health checks.
...
- **Sync Process**:
  1. Tech Lead reviews all pull requests merged in the last 30 days.
  2. Commits modifying `server/src/modules` or `web/src/app` are cross-referenced with changes to `docs/ai/`.
  3. Any missing mappings are corrected, and checklists are updated.
