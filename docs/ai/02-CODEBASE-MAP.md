...
- **Server Actions (`web/src/lib/actions/`)**: Route-independent mutation entry points (auth/profile, booking, review, rewardsAdmin, notificationsAdmin) — validate (Zod) → authorize → call backend → revalidate. Imported by both components and route files; nothing under `components/`/`lib/`/`hooks/` imports from `@/app/` (ADR-W009, lint-enforced). *Owner: Core Developer*.
- **Shared Form Controls (`web/src/components/shared/Form.js`)**: Provides the stable dashboard form primitives reused by the profile editor (`Input` and `FormAlert`) and other application forms. The profile editor owns its explicit label, description, and error associations so this shared component remains backward-compatible. *Owner: Frontend Engineer*.
...
- **Customer Dashboard (`web/src/app/(dashboard)/`)**: Renders overview, bookings, wallet, rewards, and the authenticated `/dashboard/profile` name-update form with Suspense streaming where applicable. The profile page uses the shared dashboard shell and account-settings form composition. *Owner: Frontend Engineer*.
...
| **Customer Profile Update** | `docs/specs/02-API-SPECIFICATION.md` Section 4 | `User` | `server/src/modules/users` (`PATCH /users/me`) | `web/src/app/(dashboard)/dashboard/profile` + `web/src/lib/actions/auth.js` |
