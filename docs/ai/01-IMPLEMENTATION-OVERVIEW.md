...
- **Contextual Access Control**: Roles are scoped to venues using join tables (`VenueUserRole`). A user can be a "Manager" at Venue A and a "Customer" at Venue B. Authorization is enforced at the Data Access Layer (`lib/dal/session.js`): protected **pages** call `requireRouteAccess()` with the real path (fail-closed for unmapped protected routes), owned-resource reads perform ownership checks, and every mutation action re-verifies the session — the singular effective `role` and separate `permissions` are derived from the authoritative `/users/me` response, while contextual assignments remain in `venue_roles`; client cookies are only optimistic hints.
